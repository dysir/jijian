...
{
    return base_url() . "admin/static/" . $url;
}
...
function getTopicList($aid = ""){
    $ci = & get_instance();
    $ci->load->model("Dp_article_description_model");

    $list = array();
    if(empty($aid)){
        $list = $ci->Dp_article_description_model->getTopiclist();
    }else{
        $list = $ci->Dp_article_description_model->getTopicByArticleId($aid);
    }
